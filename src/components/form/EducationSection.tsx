...
export const EducationSection = ({ form, onAdd, onRemove }: EducationSectionProps) => {
  const { control, register } = form;
  const { fields } = useFieldArray({ control, name: "education" });

  return (
    <SectionCard title="Education" description="Add academic background and achievements.">
        ...
        {fields.map((field, index) => (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">Education {index + 1}</p>
              {fields.length > 1 ? (
                <button
                  type="button"
